...
function ExamButton ({questionCount,maxCount,resultData,functions}:ExamButtonProps) {
  ...
  const actvieStyle = "bg-blue-500 text-white  hover:bg-blue-600 rounded-xl px-6 py-3 font-semibold"
    return(
                 <div className="mt-8 flex justify-between">
            <button
              disabled={questionCount == 0}
              className={questionCount == 1 ? disabledStyle : actvieStyle}
            ...
            {resultData.userAnswer.length == maxCount && maxCount == questionCount ? (
              ...
              disabled= {questionCount == maxCount}
                className={questionCount  == maxCount ? disabledStyle : actvieStyle}
